...
class Game {
  ...
  allObjects() {
    return [].concat(this.ships, this.ufos, this.shipLines, this.captures.reverse());
  }
        ...
        if (obj1.isCollidedWith(obj2)) {
          ...
          if (collision) return;
            ...
            if (obj1.isDrawing && this.shipLines.length > 50) {
              let stacked = true;
              this.addCapture(this.shipLines.slice(-1)[0].pos, this.shipLines[0].pos, stacked, this.shipLines[0].shipDir, this.shipLines.slice(-1)[0].shipDir)
              this.completeLine()
            } else {
            obj1.canDraw = true;
          }
    ...
    if (this.ships[0].isDrawing &&
      ...
      this.shipLines[0].isCollidedWith(this.shipLines.slice(-1)[0])) {
        ...
        this.isOutOfBounds(this.shipLines.slice(-1)[0].pos)) {
          // console.log(this.shipLines[0].shipDir);
          this.addCapture(this.shipLines.slice(-1)[0].pos, this.shipLines[0].pos, false, this.shipLines[0].shipDir, this.shipLines.slice(-1)[0].shipDir)
          this.completeLine()
  ...
  calculateLinePivots() {
    let pivot_points = []
    this.shipLines.forEach((el1, idx) => {
      let currentSlope = Infinity;
      let el1XPos = Math.round(el1.pos[0])
      let el1YPos = Math.round(el1.pos[1])
      // console.log(el1YPos);
      for (let i = idx + 1; i < this.shipLines.length - 2; i++) {
        let el2XPos = Math.round(this.shipLines[i].pos[0])
        let el2YPos = Math.round(this.shipLines[i].pos[1])
        let newSlope = this.calculateSlope(el1XPos, el1YPos, el2XPos, el2YPos);
        // console.log(newSlope);
      }
        // console.log(Math.floor(newSlope));
        // if (Math.abs(el1XPos) - Math.abs(el2XPos) > 50 && Math.abs(el1YPos) - Math.abs(el2YPos) > 50) {
  ...
  step(delta) {
    ...
    // console.log(this.determineQuadrant(this.ships[0].pos))
    // this.hideShip();
  }
}
