...
class Capture extends MovingObject {
	...
	drawStackedCapture(ctx) {
		...
		if (this.isOnEdge(this.startPos)) {
			...
			// TODO handle case where startPoint is on existing shape
		}
	}
...
class Track {
	...
	_formatShape(pointsArr, cornerPair) {
		let allPointsX = [];
		let allPointsY = [];
	...
	_buildCornerPairs(startPoint, endPoint) {
	...
	updateTrack(newPoints) {
		...
		if (this.areOnSameEdge(startPoint, endPoint)) {
			...
			let cornerPairs = this._buildCornerPairs(startPoint, endPoint);
			let cornerPairOne = cornerPairs[0];
			let cornerPairTwo = cornerPairs[1];

			let shapeOne = this._formatShape(newPoints, cornerPairOne);
			let shapeTwo = this._formatShape(newPoints, cornerPairTwo);
