...
class MovingObject {
  ...
  isOnXTrack(pos) {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      // 103 54
      // console.log(i)
      if (this.game.allowedTrack[[pos[0] + i, pos[1] - Math.floor(i/2)]] === true) {
        // console.log("Found Pos", [pos[0] + 1, pos[1] + i]);
        return true;
      } else if (this.game.allowedTrack[[pos[0] - i, pos[1] - Math.floor(i/2)]] === true){
        // console.log("Left X");
        // return true;
      }
    }
  ...
  power(impulse, k) {
    if (this.canDraw === false) {
      ...
      if (Util.direction(this.vel) === 'up' && k === 's' &&
        this.isOnYTrack([this.pos[0], this.pos[1] - 1])) {
        ...
        this.isOnXTrack([this.pos[0] + 1, this.pos[1]])) {
        this.vel[0] = impulse[0];
        this.vel[1] = impulse[1];
        // this.pos[0] += 1
