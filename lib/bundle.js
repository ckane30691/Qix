...
class Capture extends MovingObject {
	...
	calculateArea() {
		let shape = this.track.formatShape(this.allPoints);
		return Math.abs(Util.getArea(...shape));
	}
	...
	_getRemainingPoints() {
		if (this.track.areOnSameEdge(this.startPos, this.endPos)) return;
		if (!this.track.oppositeEdges(this.startPos, this.endPos)) {
			this.allPoints = this.allPoints.concat([
				this.track.determineCorner(this.startPos, this.endPos)
			]);
			// debugger;
			return;
		} else {
			...
			let shapeOne = this.track.areOnSameEdge(
				cornerPairOne[0],
				this.endPos
			)
				? this.allPoints.concat(cornerPairOne)
				: this.allPoints.concat(cornerPairOne.reverse());

			let shapeTwo = this.track.areOnSameEdge(
				cornerPairTwo[0],
				this.endPos
			)
				? this.allPoints.concat(cornerPairTwo)
				: this.allPoints.concat(cornerPairTwo.reverse());

			let formattedShapeOne = this.track.formatShape(shapeOne);
			let formattedShapeTwo = this.track.formatShape(shapeTwo);
			let shapeOneArea = Util.getArea(...formattedShapeOne);
			let shapeTwoArea = Util.getArea(...formattedShapeTwo);
			// debugger;
			if (Math.abs(shapeOneArea) <= Math.abs(shapeTwoArea)) {
				this.allPoints = shapeOne;
			} else {
				this.allPoints = shapeTwo;
			}
		}
	}
...
class Game {
	...
	addUfos() {
		for (let i = 0; i < Game.NUM_UFOS; i++) {
			let enemy = new Ufo({ game: this });
			this.add(enemy);
			setTimeout(() => (enemy.vel = Util.randomVec(4)), 75);
			// enemy.vel = Util.randomVec(4);
		}
	}
...
class Track {
	...
	formatShape(pointsArr) {
		...
		let xPointArray = [].concat(allPointsX, allPointsX[0]);
		let yPointArray = [].concat(allPointsY, allPointsY[0]);
		// debugger;
	...
	updateTrack(newPoints) {
		...
		if (this.areOnSameEdge(startPoint, endPoint)) {
			...
			// need to adjust this so that it works for all points passed in
			...
			let shapeOne = this.formatShape(newPoints);
			let shapeTwo = this.formatShape(newPoints);
