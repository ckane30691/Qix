...
class Capture extends MovingObject {
  ...
  getRemainingPoints() {
    ...
    // TODO: Pass in as determineCorner from Game and Track
  ...
  checkLineComplete() {
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (capture.intersects(endOfDrawnLinePos)) {
        ...
        this.addCapture(
          this.shipLines[0].startPos,
          endOfDrawnLinePos,
          true,
          this.shipLines[0].shipDir,
          this.shipLines.slice(-1)[0].shipDir,
          vertices,
          capturePoints
        );
        this.completeLine();
    ...
    if (
      ...
      this.isOutOfBounds(endOfDrawnLinePos)
    ) {
      // console.log(this.isOutOfBounds(this.shipLines.slice(-1)[0].pos));
      let vertices = this.getVertices();
      this.addCapture(
        this.shipLines[0].startPos,
        endOfDrawnLinePos,
...
class Ship extends MovingObject {
	constructor(options) {
		...
		this.startPos = null;
		this.prevPos = null;
		console.log('initial pos', this.pos);
	}
...
class Track {
  ...
  _buildCornerPair(startPoint, endPoint, flag) {
    if (flag === "left") {
  ...
  updateTrack(newPoints) {
    ...
    if (this.areOnSameEdge(startPoint, endPoint)) {
      ...
      // Need to handle edge case for two corners
      // TODO: Write buildShape
      // TODO: Write getArea
      // TODO: Refactor capture to use the methods from here
      // TODO: Refactor capture have all points up to date before draw is called
      // TODO: Handle case where you can move along edges of connected shapes
      let cornerOnePair = this._buildCornerPair(startPoint, endPoint, "left");
      let cornerTwoPair = this._buildCornerPair(startPoint, endPoint, "right");
      let shapeOneArea = getArea(cornerOnePair);
      let shapeTwoArea = getArea(cornerTwoPair);
      if (shapeOneArea <= shapeTwoArea) {
        _deleteOldTrack(startPoint, cornerOnePair[0]);
        _deleteOldTrack(endPoint, cornerOnePair[1]);
      } else {
        _deleteOldTrack(startPoint, cornerTwoPair[0]);
        _deleteOldTrack(endPoint, cornerTwoPair[1]);
      }
    }
  }
