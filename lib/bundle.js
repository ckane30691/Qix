...
const Util = __webpack_require__(0);
const Capture = __webpack_require__(3);
const Track = __webpack_require__(9);
...
class Game {
	...
	deleteOutDatedTrack(startPos, endPos) {
		if (startPos[0] === endPos[0] || startPos[1] === endPos[1]) {
			Object.keys(this.allowedTrack).forEach((allowedPos) => {
				if (
					allowedPos[1] >= startPos[1] - 3 &&
					allowedPos[1] <= startPos[1] + 3
				) {
					if (
						allowedPos[0] > startPos[0] &&
						allowedPos[0] < endPos[0]
					) {
	...
	getAllPoints() {
		...
		this.shipLines.map((shipLine) => {
			result.push([
				Math.round(shipLine.pos[0]),
				Math.round(shipLine.pos[1])
			]);
		});
		return result;
	}
			...
			for (let j = 0; j < allObjects.length; j++) {
				...
				if (obj1.isCollidedWith(obj2)) {
					...
					if (this.ships[0].isDrawing && this.shipLines.length > 5) {
						...
						this.addCapture(
							this.shipLines[0].pos,
							this.shipLines.slice(-1)[0].pos,
							stacked,
...
class Ship extends MovingObject {
	...
	move(timeDelta) {
		...
		// let tileWidth = Math.round(this.game.constructor.DIM_X / columns),
		// 	tileHeight = Math.round(this.game.constructor.DIM_Y / rows);
		// console.log('tilesize', tileWidth, tileHeight);
		// let x = this.endPos[0] * tileWidth,
		// 	y = this.endPos[1] * tileHeight;
		// console.log('x & y', x, y);
