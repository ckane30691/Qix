...
} = require("./game_helpers/line_completed_helpers/check_line_complete_helper");
const {
  addCaptureHelper,
} = require("../capture/capture_helpers/add_capture_helper");
const {
...
// WORKS CORRECTLY WHEN ADDING CAPTURE STARTING AND ENDING ON EDGE
// WORKS CORRECTLY WHEN STARTING CAPTURE ON CAPTURE AND ENDING ON SAME EDGE
// BROKEN WHEN STARTING CAPTURE ON CAPTURE AND ENDING ON DIFFERENT EDGE
// BROKEN WHEN STARTING CAPTURE ON CAPTURE THAT ITSELF ENDS ON DIFFERENT EDGE
// BROKEN WHEN ENDING CAPTURE ON CAPTURE
// WIN CONDITION
// ENEMIES SPAWN OFF GRID
...
class Game {
  ...
  // Gets called on every tick to see if line is completed and a capture should be added
  checkLineComplete() {
